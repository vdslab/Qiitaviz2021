...
function SimpleLineChart() {
  ...
  return (
    <div>
      <svg width="400" height="400" viewBox="0 0 400 400" class="graph">
          ...
          nodes.map((node) => {
            return (
              node.childNodes.map(child => {
                ...
                return (
                  <g key={node.id * 10 + target.id} transform={`rotate(${angle}, ${x1}, ${y1}) translate(${r1} , 0)`}>
                    <line x1={x1} y1={y1} x2={x1 + d} y2={y1}
                      stroke="#000" markerEnd="url(#arrow)" strokeWidth="2"
                    ></line>
                  </g>
                )
              })
